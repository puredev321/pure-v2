...
#if defined(NDEBUG)
#error "Pure cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 200 * COIN;
    } else if (nHeight >= Params().LAST_POW_BLOCK() && nHeight < 530000) {
        nSubsidy = 20 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight >= 530000 && nHeight < 775000) {
        ret = blockValue * 7 / 10;
    } else if (nHeight >= 775000) {
        ret = blockValue * 4 / 5;
    }
