...
int GetBudgetPaymentCycleBlocks()
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) return 60 * 60 * 24 * 30 / Params().TargetSpacing();
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    if (nHeight >= Params().LAST_POW_BLOCK() && nHeight < 530000) {
        nSubsidy = 2 * COIN;
    } else if (nHeight >= 530000 && nHeight < 775000) {
        nSubsidy = 1 * COIN;
    ...
    // Amount of blocks in a months period of time (using 1 minutes per) = (60*24*30)
    return nSubsidy * 30 * 24 * 60 * 60 / Params().TargetSpacing();
}
void CBudgetManager::NewBlock()
{
    ...
    if (chainActive.Height() % 10 != 0) return;
